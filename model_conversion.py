...
def dm3rep_initialize(model_num, data_type, area, thin, iter, path, bare_bones=False):
    '''
    Parameters
    ----------
    model_num : int
    ...
    if bare_bones == True: os.system('bin/fit.sh %s %s' %(model_num, c_data.convert_data_type(data_type))) # creates brad's default files
    else:
        # load data structure
